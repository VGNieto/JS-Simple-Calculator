document.querySelectorAll(".boton").forEach(function(el) {
  el.addEventListener("click", function() {
    escribir(el.getAttribute("data-tipo"), el.value);
  });
});
...
function escribir(tipo, valor) {
  ...
  if (tipo === "borrarultimo") {
    ...
    if ("+-*/".indexOf(pantalla.value.charAt(pantalla.value.length - 1)) > -1) {
      introducirOperador = false;
      introducirDecimal = true;
      introducirMasMenos = true;
      introducirUltimo = true;
      ...
      ".".indexOf(pantalla.value.charAt(pantalla.value.length - 1)) > -1
    ) {
      introducirOperador = false;
